#Initial August 2017 investigation into avg 31 days user returns spikes in Indonesia and Bangladesh.
...
##Long-term avg returns within 31 days [August 20, 2016 until May 28, 2018] 

user_returns_31days_ID_BD <- read.delim("data/user_returns_allwikis_31days_bycountry.tsv", sep = "\t", stringsAsFactors =FALSE)
user_returns_31days_ID_BD$last_seen_date <- as.Date(user_returns_31days_ID_BD$last_seen_date, format = "%d-%b-%Y")


## Bangladesh avg returns within 31 days from 2016

user_returns_bd_31days<-  user_returns_31days_ID_BD %>%
  dplyr::filter(country == "BD")

p <- ggplot(user_returns_bd_31days, aes(x = last_seen_date, y = avg_days_till_next_access, color = access_method)) +
  geom_line() +
...
ggsave("user_returns_bangladesh_31days_from2016.png", p, path = "figures/avg_user_returns/31days", units = "in", dpi = 192, height = 6, width = 10, limitsize = FALSE)
rm(p)


## Indonesia avg returns within 31 days from 2016

user_returns_id_31days <-  user_returns_31days_ID_BD %>%
  dplyr::filter(country == "ID")

p <- ggplot(user_returns_id_31days, aes(x = last_seen_date, y = avg_days_till_next_access, color = access_method)) +
  geom_line() +
...
ggsave("user_returns_indonesia_31days_from2016.png", p, path = "figures/avg_user_returns/31days", units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)


#Plot histograms showing frequeny of next return data (days until next access) around avg user return spikes in Indonesia and Bangladesh. 


# Indonesia June 2017 spike on desktop (peak of 9.9 avg days until next access on June 22, 2017)
fig_path <- file.path("figures/return_histograms/jun_2017_id")
plot_resolution <- 192
...
fig_path <- file.path("figures/return_histograms/dec_2017_id")
plot_resolution <- 192
...
fig_path <- file.path("figures/return_histograms/feb_2018_id")
plot_resolution <- 192
...
fig_path <- file.path("figures/return_histograms_oct_2017_id")
plot_resolution <- 192
...
# Bangladesh
fig_path <- file.path("figures/return_histograms/jan_2017_bd")
plot_resolution <- 192
...
fig_path <- file.path("figures/return_histograms/dec_2017_bd")
plot_resolution <- 192
...
fig_path <- file.path("figures/return_histograms/apr_2018_bd")
plot_resolution <- 192
...
fig_path <- file.path("figures/return_histograms/nov_2017_bd")
plot_resolution <- 192
